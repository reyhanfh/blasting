<template>
    <div>
      ...
      <b-table bordered striped hover :items="filteredItems" :fields="fields" :per-page="perPage" :current-page="currentPage">
        ...
        <a :href="`/blasting/segmentasi/editsegmentasi/${data.item.id}`" class="btn btn-primary btn-sm">Edit</a>
  ...
  export default {
    ...
    {
      fetchData(){
        axios.get(this.$root.baseurl +'blasting/segmentasi/getalldata').then(response => {
      ...
      deleteItem(id) {
        if (confirm('Yakin mau menghapus data ini?')) {
          axios.get(this.$root.baseurl +'blasting/segmentasi/hapussegmentasi/' + id,{
          headers: {
          '_token': this.csrf}
        })
            .then(response => {
