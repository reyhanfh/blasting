<template>
    <div>
      ...
      <template v-slot:cell(photo)="data">
          <img :src="getPhotoUrl(data.item.photo)" alt="log" height="30">
      </template>

        <template v-slot:cell(actions)="data">
        <a :href="`/blasting/template/edittemplate/${data.item.id}`" class="btn btn-primary btn-sm">Edit</a>
  ...
  export default {
    ...
    {
      fetchData(){
        axios.get(this.$root.baseurl +'blasting/template/getalldata').then(response => {
      ...
      deleteItem(id) {
        if (confirm('Yakin mau menghapus data ini?')) {
          axios.get(this.$root.baseurl +'blasting/template/hapustemplate/' + id,{
          headers: {
          '_token': this.csrf}
        })
            .then(response => {
      ...
      getPhotoUrl(photo) {
        if (photo) {
          ...
          return this.assetUrl + 'images/hotel/' + 'no-image.jpg';
        }
      }
    },
  };
