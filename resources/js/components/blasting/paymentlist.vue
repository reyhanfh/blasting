<template>
    <div>
      ...
      <b-table bordered striped hover :items="filteredItems" :fields="fields" :per-page="perPage" :current-page="currentPage">
        ...
        <a :href="`/blasting/payment/editpayment/${data.item.id}`" class="btn btn-primary btn-sm">Edit</a>
  ...
  export default {
    ...
    filteredItems() {      
      if (this.filter) {
        return this.items.filter(item => {
          return (
            item.payment.toLowerCase().includes(this.filter.toLowerCase())
          );
        });
      } else {
        return this.items;
    ...
    {
      fetchData(){
          axios.get(this.$root.baseurl +'blasting/payment/getdatapayment').then(response => {
      ...
      deleteItem(id) {
        if (confirm('Yakin mau menghapus data ini?')) {
          axios.get(this.$root.baseurl +'blasting/payment/hapuspayment/' + id,{
          headers: {
          '_token': this.csrf}
        })
            .then(response => {
